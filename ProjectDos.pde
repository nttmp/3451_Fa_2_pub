/**************************** HEADER ****************************
 ...
 Date of submission: September 2nd 2014
*****************************************************************/
...
float xb, yb; // coordinates of base point, recorded as mouse location when 'b' is released
  boolean drag = false; //ADDED
  boolean superduper = false; //ADDED
  boolean predict = false; //P2
  TRAIL T = new TRAIL(); //ADDED
...
void setup() {               // executed once at the begining 
  ...
  myFace = loadImage("data/please.png");  // loads image from file pic.jpg in folder data, replace it with a clear pic of your face
  //myFace2 = loadImage("data/p2.png");
...
void keyPressed() { // executed each time a key is pressed: the "key" variable contains the correspoinding char, 
  ...
  if(key=='d') drag=!drag; // toggle mode ADDED
  if(key=='s') superduper=!superduper; //ADDED
  if(key=='p') predict = !predict;
  change=true;
  }
...
String title ="CS3451, Fall 2014, Project 02: Parabolic Prediction", name ="RYAN MENDES   ", name2 = "Press 'p' to enable   ", // enter project number and your name
